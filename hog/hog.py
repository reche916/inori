...
def boar_brawl(player_score, opponent_score):
    ...
    player_units_digit = player_score % 10
    return 1 if opponent_tens_digit == player_units_digit else 3 * abs(opponent_tens_digit - player_units_digit)
    '''
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for i in range(0,goal):
        for j in range(0,goal):
            if strategy(i,j) not in list_1:
                list_1.append(strategy(i,j))
    if len(list_1) == 1:
        return True
    else:
        return False
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged(*args):
        ...
        for i in range(0,times_called):
            list_1.append(original_function(*args))
        return sum(list_1)/times_called
    return averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range(1,11):
        list_1.append(make_averaged(roll_dice, times_called)(num_rolls,dice))
    return list_1.index(max(list_1))+1
'''
    max_average_score = -1
    best_num_rolls = -1
    for num_rolls in range(1, 11):
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    for num_rolls in range(0,3):
        if sus_update(num_rolls, score, opponent_score)>= 100:
            return num_rolls
